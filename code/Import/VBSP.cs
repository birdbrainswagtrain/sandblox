    ...
    class VBSP
    {
		...
		public static ImportWorld Load(string filename, float scale)
        {
            ...
            using (var reader = new BinaryReader( FileSystem.Mounted.OpenRead( filename ) ) )
            {
                ...
                for (int i = 0; i < texstring_count; i++)
                {
                ...
                for (int i = 0; i < leaf_count; i++)
                {
                    ...
                    reader.ReadInt16(); // 2 leafWaterDataID
                    ...
                    if (face.dispinfo != -1)
                    {
                        ...
                        for (int y = 0; y < XYZZY; y++)
                        {
                            ...
                            for (int x = 0; x < XYZZY; x++)
                            {
                                ...
                                try_add_block(pos_int, world, COLOR_GRASS );
                            }
                        }
                ...
                Log.Info("Gathering brushes...");
                traverse(start_node);

				Log.Info( "Voxelizing brushes...");

                int handled_count = 0;
                foreach (var brush in map_brushes)
                {
                    handled_count++;
					Log.Info( "Voxelizing brush " + handled_count + " / " + map_brushes.Count);
        ...
        static ushort[] extract_materials(VBSP_Brush brush, VBSP_Side[] sides, int[] texinfo, int[] texdata, string[] texstrings)
        {
        ...
        static void process_planes(VBSP_Plane[] planes, ImportWorld world, ushort[] materials)
        {
            ...
            while (todo.Count > 0)
            {
                ...
                for (int i = 0; i < planes.Length; i++)
                {
                    ...
                    if (Math.Abs(m) < nearest_dist && mat != 0)
                    {
                ...
                if (material != null)
                {
                    ...
                    try_add_block(pos, world, material.Value);
        ...
        static void try_add_block((int,int,int) pos, ImportWorld world, ushort material) {
